...
//  2024-01-08: Metal: Fixed memory leaks when using metal-cpp (#8276, #8166) or
//  when using multiple contexts (#7419). 2022-08-23: Metal: Update deprecated
//  property 'sampleCount'->'rasterSampleCount'. 2022-07-05: Metal: Add dispatch
...
- (id<MTLRenderPipelineState>)
  renderPipelineStateForFramebufferDescriptor:(FramebufferDescriptor*)descriptor
                                       device:(id<MTLDevice>)device;
@end
...
ImGui_ImplMetal_NewFrame(MTLRenderPassDescriptor* renderPassDescriptor)
{
  ...
  IM_ASSERT(
    bd != nil &&
    "Context or backend not initialized! Did you call ImGui_ImplMetal_Init()?");
#ifdef IMGUI_IMPL_METAL_CPP
                               ...
                               id<MTLRenderCommandEncoder> commandEncoder)
{
  ...
  if (renderPipelineState == nil)
  {
    ...
    renderPipelineState =
      [ctx renderPipelineStateForFramebufferDescriptor:ctx.framebufferDescriptor
  ...
  for (int n = 0; n < drawData->CmdListsCount; n++)
  {
    ...
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
    {
      ...
      else
      {
        ...
        MTLScissorRect scissorRect = { .x = NSUInteger(clip_min.x),
                                       .y = NSUInteger(clip_min.y),
                                       .width =
                                         NSUInteger(clip_max.x - clip_min.x),
                                       .height =
                                         NSUInteger(clip_max.y - clip_min.y) };
        [commandEncoder setScissorRect:scissorRect];

        // Bind texture, Draw
        if (ImTextureID tex_id = pcmd->GetTexID())
          [commandEncoder
            setFragmentTexture:(__bridge id<
                                 MTLTexture>)(void*)(intptr_t)(tex_id)
                       atIndex:0];
...
bool
ImGui_ImplMetal_CreateFontsTexture(id<MTLDevice> device)
{
  ImGui_ImplMetal_Data* bd = ImGui_ImplMetal_GetBackendData();
  ImGuiIO&              io = ImGui::GetIO();
  ...
  io.Fonts->SetTexID(
    (ImTextureID)(intptr_t)(__bridge void*)
      bd->SharedMetalContext.fontTexture); // ImTextureID == ImU64
...
bool
ImGui_ImplMetal_CreateDeviceObjects(id<MTLDevice> device)
{
  ImGui_ImplMetal_Data*      bd = ImGui_ImplMetal_GetBackendData();
...
- (BOOL)isEqual:(id)object
{
  ...
  if (![other isKindOfClass:[FramebufferDescriptor class]]) return NO;
